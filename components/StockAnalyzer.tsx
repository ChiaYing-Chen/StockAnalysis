...
import { Search, AlertCircle, Loader2, Star, X, Database, CalendarDays } from 'lucide-react';
import { CandleData, Interval } from '../types';
...
export const StockAnalyzer: React.FC = () => {
  ...
  const fetchDataRange = async (sym: string, period1: number, period2: number, fetchedInterval: Interval = '1d') => {
    const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${sym}?interval=${fetchedInterval}&period1=${period1}&period2=${period2}`;
  ...
  // Effect to refetch when interval changes
  useEffect(() => {
    if (displaySymbol && !loading) {
      const refetch = async () => {
        setLoading(true);
        setError(null);
        setData([]); // Clear old data to avoid mixing timeframes visually
        try {
          const querySymbol = displaySymbol;
          const end = Math.floor(Date.now() / 1000);
          let startOffset = 180 * 24 * 60 * 60;
          if (interval === '1wk') startOffset *= 3;
          if (interval === '1mo') startOffset *= 10;
          const start = end - startOffset;

          const { parsedData } = await fetchDataRange(querySymbol, start, end, interval);
  ...
  const fetchStockData = async (e?: React.FormEvent, overrideSymbol?: string) => {
    ...
    try {
      ...
      const end = Math.floor(Date.now() / 1000);
      let startOffset = 180 * 24 * 60 * 60; // 6 months default for daily
      ...
      const { parsedData, meta } = await fetchDataRange(querySymbol, start, end, interval);
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      const { parsedData } = await fetchDataRange(displaySymbol, Math.floor(newStart), Math.floor(currentStart), interval);
